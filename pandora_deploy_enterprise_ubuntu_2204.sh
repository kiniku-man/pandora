#!/bin/bash
##############################################################################################################
# PandoraFMS online installation script for Ubuntu 22.04
##############################################################################################################
## Tested versions ##
# Ubuntu 22.04.1
# Ubuntu 22.04.2
...
execute_cmd () {
    ...
    if [ $? -ne 0 ]; then
        ...
        [ "$3" ] && echo "$3"
        echo "Error installing PandoraFMS for detailed error please check log: $LOGFILE"
...
check_pre_pandora () {
    ...
    [ ! $fail ]
    check_cmd_status 'Error there is a current PandoraFMS installation on this node, please remove it to execute a clean install'
}

check_repo_connection () {
    execute_cmd "ping -c 2 packages.pandorafms.com" "Checking PandoraFMS repo"
}
...
## Main
echo "Starting PandoraFMS deployment Ubuntu 22.04 ver. $S_VERSION"
...
echo "Installer version: $S_VERSION" >> "$LOGFILE"
...
# Console dependencies
    ...
    whois \
    cron"
execute_cmd "apt install -y $console_dependencies" "Installing PandoraFMS Console dependencies"
...
server_dependencies=" \
	...
	libgeo-ip-perl \
    	...
    	traceroute \
	openjdk-8-jdk "
execute_cmd "apt install -y $server_dependencies" "Installing PandoraFMS Server dependencies"

execute_cmd "installing_docker" "Installing Docker for debug"

# Installing pandora_gotty
execute_cmd "curl --output pandora_gotty.deb https://packages.pandorafms.com/ubuntu/pandora_gotty_1.0.0.deb" "Downloading pandora_gotty"
execute_cmd "apt install -y ./pandora_gotty.deb" "Intalling pandora_gotty"

# Installing MADE
execute_cmd "curl --output pandora_made.deb https://packages.pandorafms.com/ubuntu/pandorafms-made_0.1.0-2_amd64.deb" "Downloading pandora MADE"
execute_cmd "apt install -y ./pandora_made.deb" "Intalling pandora MADE"

# wmic and pandorawmic
execute_cmd "curl -O https://packages.pandorafms.com/pandorafms/utils/bin/wmic" "Downloading wmic"
execute_cmd "curl -O https://packages.pandorafms.com/pandorafms/utils/bin/pandorawmic" "Downloading pandorawmic"
...
echo -en "${cyan}Installing Percona Server for MySQL8...${reset}"
    env DEBIAN_FRONTEND=noninteractive apt install -y percona-server-server percona-xtrabackup-80 libmariadb3 &>> "$LOGFILE"
check_cmd_status "Error Installing MySql Server"
mkdir -p /usr/lib64/mysql/plugin/  &>> "$LOGFILE"
ln -s /usr/lib/x86_64-linux-gnu/libmariadb3/plugin/caching_sha2_password.so /usr/lib64/mysql/plugin/caching_sha2_password.so  &>> "$LOGFILE"
...
if [ "$SKIP_DATABASE_INSTALL" -eq '0' ] ; then
    ...
    echo """
    ALTER USER 'root'@'localhost' IDENTIFIED WITH caching_sha2_password BY '$DBROOTPASS';
    """ | mysql -uroot &>> "$LOGFILE"

    export MYSQL_PWD=$DBROOTPASS
    echo "INSTALL COMPONENT 'file://component_validate_password';" | mysql -uroot -P$DBPORT -h$DBHOST &>> "$LOGFILE"
    echo -en "${cyan}Creating PandoraFMS database...${reset}"
    echo "create database $DBNAME" | mysql -uroot -P$DBPORT -h$DBHOST
    check_cmd_status "Error creating database $DBNAME, is this an empty node? if you have a previus installation please contact with support."

    echo "CREATE USER  \"$DBUSER\"@'%' IDENTIFIED BY \"$DBPASS\";" | mysql -uroot -P$DBPORT -h$DBHOST
    echo "ALTER USER \"$DBUSER\"@'%' IDENTIFIED WITH caching_sha2_password BY \"$DBPASS\"" | mysql -uroot -P$DBPORT -h$DBHOST        
...
symbolic-links=0
# Mysql optimizations for PandoraFMS
...
if [ "$PANDORA_LTS" -eq '1' ] ; then
    [ "$PANDORA_SERVER_PACKAGE" ]       || PANDORA_SERVER_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/LTS/pandorafms_server-lts.tar.gz"
    [ "$PANDORA_CONSOLE_PACKAGE" ]      || PANDORA_CONSOLE_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/LTS/pandorafms_console-lts.src.tar.gz"
    [ "$PANDORA_AGENT_PACKAGE" ]        || PANDORA_AGENT_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/LTS/pandorafms_one_agent_linux-lts.tar.gz"
elif [ "$PANDORA_LTS" -ne '1' ] ; then
    [ "$PANDORA_SERVER_PACKAGE" ]       || PANDORA_SERVER_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/latest/pandorafms_server-latest.tar.gz"
    [ "$PANDORA_CONSOLE_PACKAGE" ]      || PANDORA_CONSOLE_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/latest/pandorafms_console-latest.src.tar.gz"
    [ "$PANDORA_AGENT_PACKAGE" ]        || PANDORA_AGENT_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/latest/pandorafms_one_agent_linux-latest.tar.gz"
fi

if [ "$PANDORA_BETA" -eq '1' ] ; then
    PANDORA_SERVER_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/nightlies/pandorafms_server-nightlies.tar.gz"
    PANDORA_CONSOLE_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/nightlies/pandorafms_console-nightlies.src.tar.gz"
    PANDORA_AGENT_PACKAGE="https://packages.pandorafms.com/j8jbrEYE4nHZqhSZ7IGGuRxpP3Bdzm1s/nightlies/pandorafms_one_agent_linux-nightlies.tar.gz"
fi

# Downloading Pandora Packages
cd $WORKDIR &>> "$LOGFILE"
execute_cmd "curl -LSs --output pandorafms_console-7.0NG.tar.gz ${PANDORA_CONSOLE_PACKAGE}" "Downloading PandoraFMS Console"
execute_cmd "curl -LSs --output pandorafms_server-7.0NG.tar.gz ${PANDORA_SERVER_PACKAGE}" "Downloading PandoraFMS Server"
execute_cmd "curl -LSs --output pandorafms_agent_linux-7.0NG.tar.gz ${PANDORA_AGENT_PACKAGE}" "Downloading PandoraFMS agent"
 
curl -LSs --output pandorafms_server-7.0NG.tar.gz "${PANDORA_SERVER_PACKAGE}"
curl -LSs --output pandorafms_agent_linux-7.0NG.tar.gz "${PANDORA_AGENT_PACKAGE}"
...
rm -f $PANDORA_CONSOLE/enterprise/*.spec &>> $LOGFILE

# Install PandoraFMS Server
...
execute_cmd "echo 1" "Enabling PandoraFMS Enterprise Console cron task"
...
#execute_cmd "/etc/init.d/pandora_server start" "Starting PandoraFMS Server"
systemctl enable pandora_server &>> "$LOGFILE"
execute_cmd "systemctl restart pandora_server" "Starting PandoraFMS Server"

# starting tentacle server
systemctl enable tentacle_serverd &>> "$LOGFILE"
execute_cmd "systemctl restart tentacle_serverd" "Starting Tentacle Server"

# Enabling pandoradb cron
execute_cmd "echo 'enabling pandoradb cron' >> $PANDORA_CONSOLE/log/cron.log\" >> /etc/crontab" "Enabling PandoraFMS pandoradb cron"
...
execute_cmd "systemctl restart pandora_agent_daemon" "Starting PandoraFSM Agent"
...
# Disable made
systemctl disable --now  pandora_made &>> "$LOGFILE"
...
#Enable Pandora Ent free
if [ "$PANDORA_ENT_FREE" -eq '1' ] ; then
    export MYSQL_PWD=$DBPASS
    wget -q -O - --no-check-certificate --load-cookies /tmp/cron-session-cookies --save-cookies /tmp/cron-session-cookies --keep-session-cookies http://127.0.0.1/pandora_console/enterprise/cron.php >> $LOGFILE
    echo 'update tupdate_settings set `value`="PANDORA-ENTERPRISE-FREE" where `key`="customer_key"' | mysql -u$DBUSER -P$DBPORT -h$DBHOST $DBNAME &>> $LOGFILE
fi
...
Welcome to PandoraFMS appliance on Ubuntu
------------------------------------------
...
execute_cmd "echo done" "PandoraFMS installed"
cd "$HOME"
